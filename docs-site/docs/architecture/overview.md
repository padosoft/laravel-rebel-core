...
`Architecture Overview` keeps the Laravel Rebel ecosystem understandable as separate packages evolve independently. Each package owns a narrow responsibility while the core package defines the shared language of assurance, context, audit and contracts.
